...
class AdversarialCourt:
    ...
    def review(self, hypothesis: Hypothesis, ledger: EvidenceLedger, *, scope_ok: bool,
               independently_reproduced: bool, skeptic_objections: list[str] | None = None) -> CourtDecision:
        objections = [o.strip() for o in (skeptic_objections or []) if o.strip()]
        reasons: list[str] = []
        if not scope_ok:
            return CourtDecision(hypothesis.id, Verdict.REJECTED, ledger.confidence, ["scope/policy gate failed"])
        if ledger.confidence < 0.50:
            return CourtDecision(hypothesis.id, Verdict.REJECTED, ledger.confidence, ["evidence weighs against the hypothesis"])
        ...
        if ledger.confidence < self.confirmation_threshold:
            reasons.append(f"confidence {ledger.confidence:.3f} below threshold {self.confirmation_threshold:.3f}")
        if reasons:
            return CourtDecision(hypothesis.id, Verdict.PROVISIONAL, ledger.confidence, reasons)
        return CourtDecision(hypothesis.id, Verdict.CONFIRMED, ledger.confidence,
